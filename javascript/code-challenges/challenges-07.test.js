...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let sortSWC = starWarsArr.sort((a,b) => parseInt(b.height) - parseInt(a.height));
  return sortSWC;
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    let I = item.indexOf(' ');
    result.push(item.slice(item.indexOf(' ', I + 1), item.length).trim());
  });
  return result;
};
