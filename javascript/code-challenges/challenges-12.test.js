...
const validatePin = (pin) => {
  // Solution code here...
  const digitsC = String(pin).match(/([0-9])/g) ;
  const nondigits = String(pin).match(/([^0-9])/g);
  return digitsC? ( (digitsC.length === 4 && !nondigits) ? true : false) :false ;
};
...
const validateWord = (word) => {
  // Solution code here...
  if(!(typeof word == 'string') || word.match(/\d/g) ) return false ;
  const countChr = word.match(/\w/g);
  return countChr ? ( (countChr.length >= 5 && countChr.length < 10) ? true : false ) : false ;
};
...
const hasNumber = (string) => {
  // Solution code here...
  if(!(String(string).match(/\d/)) || (typeof string != 'string') ) return false ;
  return string.match(/\D/).index < string.match(/\d/).index ? true : false ;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  const validNumber = phoneNumber.match(/\(?([0-9]{3})\)?[-.●]?[\s]?([0-9]{3})[-.●]?[\s]?([0-9]{4})/g);
  if(!validNumber) return false ;
  if(validNumber[0].length !== phoneNumber.length ) return false ;
  return validNumber[0].match(/\(/) ? (validNumber[0].match(/\)/)? true : false) : true ;
};
